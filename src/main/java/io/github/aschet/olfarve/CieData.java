...
/**
 * Reference colorimetric data used by {@link Olfarve}.
 *
...
final class CieData {
  ...
  /** Wavelength of the first row in {@link #SAMPLES}, in nanometers. */
  static final double FIRST_WAVELENGTH_NM = 380.0;

  /** Distance between two consecutive rows in {@link #SAMPLES}, in nanometers. */
  static final double WAVELENGTH_STEP_NM = 5.0;

  /**
   * Colorimetric samples from 380 nm to 780 nm in 5 nm increments.
   *
   * <p>Each row is {@code {xBar, yBar, zBar, sD65}}: the color matching functions x(lambda),
   * y(lambda) and z(lambda), and the relative spectral power distribution S(lambda) of illuminant
   * D65.
   */
  static final double[][] SAMPLES = {
